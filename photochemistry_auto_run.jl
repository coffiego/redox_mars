...
#This file should be included to calc time responses of atmosphere to some changes
#temperature,water abundance, oxygen escape, pCO2,,,

### koyama comment #############################################################
#This file is modified by Koyama to investigate the regulation of water escape from Martian atmosphere
#Investigating self-regulation (redox re-equilibration) timescale for different conditions of early Mars
#Changing atmospheric surface pressure (pCO2), surface temperature, O escape rate.
...
#2
#Though HOCO plays a role in recombination of CO2,
#it is removed because the model becomes unstable with denser atmospheres.
#Additional loss of HOCO reactions would be needed in denser atmosphere?

#3
# position of CO2+, exobase altitude are positioned at same atmospheric
# pressure level in denser atmospheres

#4
# Imposing depostion velocity on reactive species(mainly H2O2, O3)

############################################

#here is parameters to investigate regulation regulationtimescale
CO2_pressure = 10#mbar
surface_temperature = 210#K
Oxygen_escape_rate = 2.4e8#cm-2s-1
...
folder_directory = "/Users/shungokoyama/Programming/result/stat/"
...
elseif CO2_pressure < 4000
    ...
    readfile = "/Users/shungokoyama/Programming/result/stat/conv_Tmod_Tsurf"*string(surface_temperature)*
        "_Oesc"*string(Oxygen_escape_rate_before)* "/converged_Tsurf_" * string(surface_temperature) * "_CO2_" *
        string(CO2_pressure) * "mbar_Oesc_" * string(Oxygen_escape_rate_before) *
        "_depv_" * string(depv) * "_nofixedCO2_noHOCO_Tmod_1billion.h5"
else
    readfile = folder_directory *"conv_Tsurf"*string(surface_temperature)*"_Oesc"*string(Oxygen_escape_rate_before)*
                "Tmod/converged_Tsurf_" * string(surface_temperature) * "_CO2_" *
                string(CO2_pressure) * "mbar_Oesc_" * string(Oxygen_escape_rate_before) *
                "_depv_" * string(depv) * "_nofixedCO2_noHOCO_Tmod_10billion.h5"
...
#function Tpiecewise(z::Float64, Tinf=240.0, Ttropo=116.0, lapserate=-1.4e-5, ztropo=90e5, ztropowidth=30e5) #Koyama T = 200K
#function Tpiecewise(z::Float64, Tinf=240.0, Ttropo=125.0, lapserate=-1.4e-5, ztropo=90e5, ztropowidth=30e5) #original Chaffin(Tsurf=209K)
#function Tpiecewise(z::Float64, Tinf=240.0, Ttropo=136.0, lapserate=-1.4e-5, ztropo=90e5, ztropowidth=30e5)# koyama change T=220K
#function Tpiecewise(z::Float64, Tinf=240.0, Ttropo=146.0, lapserate=-1.4e-5, ztropo=90e5, ztropowidth=30e5) #koyama Tsurf = 230K
#function Tpiecewise(z::Float64, Tinf=240.0, Ttropo=166.0, lapserate=-1.4e-5, ztropo=90e5, ztropowidth=30e5) #koyama Tsurf = 250K
#function Tpiecewise(z::Float64, Tinf=240.0, Ttropo=186.0, lapserate=-1.4e-5, ztropo=90e5, ztropowidth=30e5) #koyama Tsurf = 270K
...
#3次間数で繋げようとしたけどなんかだめ
#LH = [length(H2Osatfrac_lowalt)^3 length(H2Osatfrac_lowalt)^2 length(H2Osatfrac_lowalt) 1;
#    length(H2Oinitfrac)^3 length(H2Oinitfrac)^2 length(H2Oinitfrac) 1;
#    3*length(H2Osatfrac_lowalt)^2 2*length(H2Osatfrac_lowalt) 1 0;
#    3*length(H2Oinitfrac)^2 2*length(H2Oinitfrac) 1 0]
#RH = [H2Osatfrac_lowalt[end],minimum(H2Oinitfrac),H2Osatfrac_lowalt[end]-H2Osatfrac_lowalt[end-1],-1e10]

#aa,bb,cc,dd = LH \ RH
#for i in [length(H2Osatfrac_lowalt)+1:length(H2Oinitfrac)-1;]
#    append!(H2Osatfrac_midalt, aa*i^3+bb*i^2+cc*i+dd)#y = 10^(aa*z + bb)
#end
...
const speciesbclist=Dict(
               #:CO2=>["n" 2.1e17; "f" 0.], #normal:2.1e17 koyama
               #:Ar=>["n" 2.0e-2*2.1e17; "f" 0.],
               #:N2=>["n" 1.9e-2*2.1e17; "f" 0.],
               #:H2O=>["n" H2Osat[1]; "f" 0.], # boundary condition does
                                              # not matter if H2O is
                                              # fixed
               #:CO=>["f" 0.; "f" 1.0e10], #koyama added just curious
               :O=>["v" depv; "f" Oxygen_escape_rate],#normal:1.2e8
               :H2=>["f" 0.; "v" H2_effusion_velocity],
               :H=>["v" depv; "v" H_effusion_velocity],
               #:H=>["v" 0.; "v" H_effusion_velocity],
...
function boundaryconditions(species, dz, n_current)
    ...
    if bcs[2, 1] == "n"
        bcvec[2,:] = [fluxcoefs(alt[end-1],dz, species, n_current)[2],
                    upper_down(alt[end],dz, species, n_current)*bcs[2, 2]] #I fixed it from bcs[1,2] to bcs[2,2]
    elseif bcs[2, 1] == "f"
...
function next_timestep(nstart::Array{Float64, 1},
   ...
   # coupled transport and chemical reaction network.
	...
	updatemat = chemJmat(nthis, inactive, Jrates, T, M, tup, tdown, tlower,tupper, dt)+0.5*eye(length(nthis))
        # update
...
function plotTemp()
    ...
    xlim(120, surface_temperature+10)
    xticks(size = 5)
    yticks(size = 5)
    xlabel("Temperature[K]",size=15)
    ylabel("Altitude[km]",size=15)
    title("Temperature")
end
...
function plotH2O()
    ...
    xticks(size = 25)
    yticks(size = 25)
    xlabel("Species concentration [/cm3]",size = 20)
...
function printkey()
    Hesc = H_effusion_velocity*n_current[:H][end]+2*H2_effusion_velocity*n_current[:H2][end]
    COmixing = sum(n_current[:CO])/sum(map(z->n_tot(n_current, z),alt[1:end-2]))
    println(totaltime)
    println("Hesc: ", Hesc)
    println("CO mixing ratio: ",sum(n_current[:CO])/sum(map(z->n_tot(n_current, z),alt[1:end-2])))
    println("H2O2 dep flux: ",n_current[:H2O2][1]*0.02)
    println("O3 dep flux: ", n_current[:O3][1]*0.02)
    println("HO2 dep flux:", n_current[:HO2][1]*0.02)
    println("HOCO dep flux: ",n_current[:HOCO][1]*0.02)
    Htotalflux = Hesc+n_current[:H2O2][1]*depv*2+n_current[:HO2][1]*depv+n_current[:H][1]*depv+n_current[:OH][1]*depv
    Ototalflux = speciesbclist[:O][2,2]+n_current[:H2O2][1]*depv*2+n_current[:O3][1]*depv*3+n_current[:HO2][1]*depv*2 +n_current[:OH][1]*depv + n_current[:O][1]*depv
...
if get_converged
    ...
    written_file_name =  folder_directory *
                        "converged_Tsurf_" * string(surface_temperature) * "_CO2_" *
                        string(CO2_pressure) * "mbar_Oesc_" * string(Oxygen_escape_rate) *
                        "_depv_" * string(depv) * "_nofixedCO2_noHOCO_Tmod"
    ...
    write_ncurrent(n_current,written_file_name*"_1billion.h5") #save 1 Gyr model run
